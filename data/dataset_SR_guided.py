...
class Dataset(data.Dataset): 
    ...
    def __getitem__(self, index):
        ...
        if self.paths_lr:
            ...
            # print(img_Lr.dtype, img_Lr.shape, '-' * 10)
            img_Lr = np.clip(util.imresize_np(img_Lr.astype(np.float32), self.sf, True), 0, 255)
        ...
        if self.opt['phase'] == 'test': 
            ...
            # target = (640, 448)
            img_Lr,img_guide = (
                util.uint2tensor3(util.imresize_np(img_Lr.astype(np.float32), self.sf, True)), util.uint2tensor3(img_guide)
            )
