...
class IRSRMamba(nn.Module):
    ...
    def __init__(self,
        ...
        self.featureFusionmodule = FeatureFusionModule(num_in_ch, num_out_ch, upscale, num_in_ch)
    ...
    def forward(self, samples):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            # print("featureFusionmodule Using")
            x = self.conv_first(x)
        ...
        return {'img_out': x + samples['lr_up']}
...
def make_model(args): return IRSRMamba(upscale=args.scale)
...
if __name__ == '__main__':
    ...
    sample = {
        'lr_img': torch.randn(1, 1, 32, 32).cuda(),
        'lr_up': torch.randn(1, 1, 256, 256).cuda(),
        'img_rgb': torch.randn(1, 3, 256, 256).cuda()
    }
    net = IRSRMamba(upscale=args.scale).cuda()
    total = sum([param.nelement() for param in net.parameters()])
    print("Number of parameter: %.3fM" % (total / 1e6))
    print(net(sample)['img_out'].size())